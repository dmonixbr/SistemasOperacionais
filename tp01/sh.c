...
/* MARK NAME Victor Vieira de Melo */
/* MARK NAME E Etc */
...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
     ...
     * O if apresentado abaixo checa se a entrada do shell é um cd.
     * no segundo if é executado o comando chdir, que muda o diretorio atual do shell
     * caso o diretório não seja válido, a função chdir retorna -1 e então
     * joga um erro na tela pela saída padrão.
     * */
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      buf[strlen(buf)-1] = 0;
      if(chdir(buf+3) < 0)
        fprintf(stderr, "cd: Diretorio inexistente!\n");
      continue;
    }
    /* MARK END task1 */
